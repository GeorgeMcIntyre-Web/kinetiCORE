...
## Converted USD Libraries (GLB) Integration

You can browse and load GLB assets generated by the production converter (`usd_to_glb_converter.py`).

Configuration options:
- Create `public/assetsources.json` with a list of roots (optional):

```json
{
  "roots": [
    "C:/Users/georgem/source/repos/usd/Warehouse_NVD@10013_converted",
    "C:/Users/georgem/source/repos/usd/Scene_Templates_NVD@10011_converted"
  ]
}
```

- Or set environment variable for the USD Flask server (dev):
  - `ASSET_ROOTS="C:/.../Warehouse_NVD@10013_converted;C:/.../Scene_Templates_NVD@10011_converted"`

Run:
1) npm install
2) npm run dev
3) Start the USD server (when importing USD):
   - Windows: `npm run usd:server:win`
   - Any OS: `npm run usd:server`
   - Health: `http://localhost:5001/api/health`

Usage:
- Open the Asset Library panel and search for GLB assets (loaderType `glb`).
- Double-click any GLB to load. The model is wrapped under a parent node, orientation corrected, and the camera frames it.
