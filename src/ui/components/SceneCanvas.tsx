  ...
  return (
    ...
    >
      ...
      <div className="absolute top-56 left-4 flex flex-col gap-1 z-50">
        {/* Quick snap toggle */}
        <button
          className="w-11 h-11 flex items-center justify-center bg-gray-900 bg-opacity-95 border border-gray-600 rounded-lg hover:bg-green-600 transition-all"
          onClick={() => useEditorStore.getState().setSnapEnabled(!useEditorStore.getState().snapEnabled)}
          title={`Snapping: ${useEditorStore.getState().snapEnabled ? 'ON' : 'OFF'}`}
          style={{
            background: useEditorStore.getState().snapEnabled ? '#48bb78' : 'rgba(26, 32, 44, 0.95)',
          }}
        >
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-gray-200">
